...
public class HtmlCaptchaChecker implements IHtmlCaptchaChecker {
    // private static final String CHECK_URI_FORMAT =
    // "http://2ch.so/%s/wakaba.pl?task=captcha&thread=%s&dummy=";
    // private static final String HTML_RESPONSE_SKIP_CAPTCHA =
    ...
    public boolean canSkipCaptcha(Uri refererUri) {
        ...
        // Add referer, because it always returns the incorrect value CHECK if
        // not to set it
    ...
    public boolean checkHtmlBlock(String captchaBlock) {
         ...
         * if(text.contains(HTML_RESPONSE_SKIP_CAPTCHA)){ return true; }
         */
